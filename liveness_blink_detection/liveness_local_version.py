...
class BlinkEyeDetection:
	...
	def detect_eye_blink(self, video_path, store_image_path = "temp.jpg", STD_THRESH = 0.015):
		...
		metadata = pyexifinfo.get_json(video_path)
		try:
			rotation_value = metadata[0]['Composite:Rotation']
		except Exception as e:
			rotation_value = 0		
		...
		while True:
			...
			#if ear > max_ear:
			if	np.std(eyeaspectratiolist) < STD_THRESH and ear > max_ear:
				max_frame = frame
